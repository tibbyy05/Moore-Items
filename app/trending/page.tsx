...
export default function TrendingPage() {
  ...
  const fetchTrending = async (pageToLoad: number, replace: boolean) => {
    if (replace) {
      setLoading(true);
    } else {
    ...
    const response = await fetch(
      `/api/products?sort=best-selling&limit=${LIMIT}&page=${pageToLoad}${totalParam}`
    );
  ...
  return (
    <>
      <PaginationHead currentPage={page} totalPages={totalPages} basePath="/trending" />
      <Header />
      <CartDrawer />
        ...
        <div className="bg-warm-50 border-b border-warm-200 py-6">
          <div className="max-w-[1600px] mx-auto px-4">
            <nav className="flex items-center gap-2 text-sm mb-4">
              ...
              <span className="text-warm-900 font-medium">Trending Now</span>
            </nav>
              ...
              {loading && products.length === 0 ? (
                ...
                <div className="text-center py-16">
                  ...
                  <p className="text-warm-600 mb-6">
                    Try adjusting your filters or explore new arrivals.
                  </p>
