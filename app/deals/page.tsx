...
import { ChevronRight, SlidersHorizontal, Grid3x3, List, Tag } from 'lucide-react';
...
export default function DealsPage() {
  ...
  const fetchDeals = async (pageToLoad: number, replace: boolean) => {
    if (replace) {
      setLoading(true);
    } else {
    ...
    if (response.ok) {
      ...
      const deals = mapped.filter(
        (item: Product) => item.compareAtPrice && item.compareAtPrice > item.price
      );
      setProducts((prev) => (replace ? deals : [...prev, ...deals]));
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        case 'featured': {
          const aDiscount = a.compareAtPrice
            ? (a.compareAtPrice - a.price) / a.compareAtPrice
            : 0;
          const bDiscount = b.compareAtPrice
        ...
        <div className="bg-warm-50 border-b border-warm-200 py-6">
          <div className="max-w-[1600px] mx-auto px-4">
            <nav className="flex items-center gap-2 text-sm mb-4">
              ...
              <span className="text-warm-900 font-medium">Deals & Sales</span>
            </nav>
              ...
              {loading && products.length === 0 ? (
                ...
                <div className="text-center py-16">
                  <div className="w-20 h-20 mx-auto mb-6 rounded-full bg-warm-50 flex items-center justify-center">
                    <Tag className="w-10 h-10 text-warm-400" />
                  </div>
                  <h3 className="text-xl font-playfair font-semibold text-warm-900 mb-2">
                    No deals right now
                  </h3>
                  <p className="text-warm-600 mb-6">Check back soon for new savings.</p>
