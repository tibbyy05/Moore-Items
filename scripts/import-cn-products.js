// Run with: node scripts/import-cn-products.js
// Imports CN warehouse products from CJ into Supabase
...
function calculateRetailPrice(cjPrice, shippingCost) {
  ...
  const retail = (2.2 * (base + 0.3)) / 0.942;
...
async function run() {
  ...
  if (!Number.isFinite(startPage) || !Number.isFinite(endPage) || endPage < startPage) {
    console.log('[import] Invalid page range. Usage: node scripts/import-cn-products.js 2 10');
    return;
  }
  ...
  for (let page = startPage; page <= endPage; page += 1) {
    ...
    const listRes = await fetch(
      `${BASE_URL}/product/listV2?page=${page}&size=100&orderBy=1&sort=desc`,
    ...
    for (const product of products) {
      ...
      try {
        ...
        const payload = {
          ...
          shipping_estimate: '7-15 business days',
          available_warehouses: ['CN'],
  ...
  console.log('[import] REMINDER: Run ai-categorize-remaining.js to categorize new products (do NOT use keyword scripts)');
}
